<script lang="ts">
	...
	// Fixed glyph + color per kind so the body column starts at a consistent x
	// regardless of how wide an individual glyph renders in the mono font.
	const glyph = $derived(
		({
			commits: { ch: '↑', color: 'text-primary' },
			pr_opened: { ch: '+', color: 'text-crt-warn' },
			pr_merged: { ch: '✓', color: 'text-crt-ok' },
			pr_closed: { ch: '✕', color: 'text-destructive' },
			release: { ch: '◆', color: 'text-crt-ok' },
			issue_opened: { ch: '⚠', color: 'text-crt-warn' },
			issue_closed: { ch: '✓', color: 'text-crt-ok' },
			comment: { ch: '›', color: 'text-crt-link' },
		} as const)[ev.kind],
	);
</script>

<!-- Flex row: fixed-width timestamp + glyph columns, then a wrapping body whose
     wrapped continuation stays aligned under the body (not under the timestamp). -->
<div class="bootln flex items-baseline gap-2 text-[13px] leading-[1.75]">
	<span class="text-crt-faint shrink-0 tabular-nums">{time}</span>
	<span class={`shrink-0 w-4 text-center ${glyph.color}`} aria-hidden="true">{glyph.ch}</span>
